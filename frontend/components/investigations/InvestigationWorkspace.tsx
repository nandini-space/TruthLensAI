...
export function InvestigationResults({ result }: { result: Module2Result }) {
  ...
  return <div className="results">
    ...
    <Section title="Forensic report">{report ? <><dl className="detail-list"><div><dt>Report ID</dt><dd className="mono">{report.report_id}</dd></div><div><dt>Generated</dt><dd>{report.generated_at}</dd></div><div><dt>Recommended action</dt><dd>{report.recommended_action}</dd></div></dl><p>{report.analysis}</p></> : <EmptyState>No forensic report was returned.</EmptyState>}</Section>
    <Section title="STIX 2.1 bundle">{result.stix_bundle ? <p className="success">A STIX bundle was returned with this in-memory investigation. No download endpoint is currently available.</p> : <EmptyState>No STIX bundle was returned.</EmptyState>}</Section>
  </div>;
}
